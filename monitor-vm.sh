...
check_disk_initialization() {
    ...
    if [ -f "/isos/xp.vhd" ]; then
        ...
        if dd if=/isos/xp.vhd bs=1 skip=510 count=2 2>/dev/null | grep -q $'\x55\xaa'; then
            echo "✅ Valid boot sector detected"
            
            # Check partition table and file system
            local partition_info=$(parted /isos/xp.vhd print 2>/dev/null | grep -E 'primary.*ntfs.*boot')
            if [ -n "$partition_info" ]; then
                ...
                local vhd_size_mb=$(du -m /isos/xp.vhd | cut -f1)
...
check_disk_activity() {
    ...
    # VHD file info
    if [ -f "/isos/xp.vhd" ]; then
        local size=$(du -h /isos/xp.vhd | cut -f1)
        local timestamp=$(stat -c %Y /isos/xp.vhd)
...
estimate_phase() {
    ...
    local vhd_size_mb=0
    if [ -f "/isos/xp.vhd" ]; then
        vhd_size_mb=$(du -m /isos/xp.vhd | cut -f1)
    fi
